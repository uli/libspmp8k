...
void DoJoystick_L()
{
	...
	if (KeyboardEnabled)
	{
		if (keys.keys & GE_KEY_RIGHT) IOPortA = IOPortA & 0x7f;
		if (keys.keys & GE_KEY_LEFT) IOPortA = IOPortA & 0xbf;
		if (keys.keys & GE_KEY_DOWN) IOPortA = IOPortA & 0xdf;
		if (keys.keys & GE_KEY_UP) IOPortA = IOPortA & 0xef;
		if (keys.keys & GE_KEY_X) InputLatch[0] = 0x00;
